...
from pythia.iterative_prewhitening import run_ipw
from pythia.periodograms import LS_periodogram
import os

if __name__=="__main__":
  print(os.getcwd())

  times,fluxes = np.loadtxt("test/test_kepler_pulsator.txt",unpack=True)
  ...
  np.savetxt('test/test_kepler_pulsator.out',np.array([offsets,frequencies,amplitudes,phases]).T)
  fig,ax = plt.subplots(1,1,figsize=(6.6957,6.6957))
